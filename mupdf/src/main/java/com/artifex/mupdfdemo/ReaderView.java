...
public class ReaderView
	...
	private static final long ANIMATED_ZOOM_DURATION = 500; //ms
	private static final int SCROLL_DURATION = 400; //ms
	...
	private AtomicBoolean mShouldZoomOutAfterUserInteraction = new AtomicBoolean(false);
	...
	public void smartMoveForwards() {
		...
		mScroller.startScroll(0, 0, remainingX - xOffset, remainingY - yOffset, SCROLL_DURATION);
		mStepper.prod();
	}
	...
	public boolean onDoubleTap(final MotionEvent e) {
		...
		animatedScale(e.getX(), e.getY(), false);
	...
	private void animatedScale(final float focusX, final float focusY, boolean forceZoomOut) {
		float scale_factor = mReflow ? REFLOW_SCALE_FACTOR : 1.0f;
		float min_scale = MIN_SCALE * scale_factor;
		float max_scale = MAX_SCALE * scale_factor;

		boolean zoomIn = mScale >= max_scale / 2.0f || forceZoomOut ? false : true;

		float startScale = mScale;
		float targetScale = zoomIn ? max_scale : min_scale;

		ValueAnimator animation = ValueAnimator.ofFloat(startScale, targetScale);
		animation.setDuration(ANIMATED_ZOOM_DURATION);
		...
		animation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
			...
			public void onAnimationUpdate(ValueAnimator animation) {
				...
				if (mReflow) {
					...
					View v = mChildViews.get(mCurrent);
					if (v != null) {
						float currentFocusX = focusX;
						float currentFocusY = focusY;
	...
	public void zoomOut() {
		if(!mUserInteracting) {
			animatedScale(getMeasuredWidth() / 2.0f, getMeasuredHeight() / 2.0f, true);
		}
		else {
			mShouldZoomOutAfterUserInteraction.set(true);
		}
	}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if ((event.getAction() & MotionEvent.ACTION_MASK) == MotionEvent.ACTION_UP) {
			...
			if(mShouldZoomOutAfterUserInteraction.get()) {
				mShouldZoomOutAfterUserInteraction.set(false);
				zoomOut();
			}
		}
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		try {
			...
			Logger.warn(e, "Out of memory during layout");
			Broadcast.postEvent(new OutOfMemoryError("Out of memory during layout"));
		}
	}
	...
	private void slideViewOntoScreen(View v) {
		...
		if (corr.x != 0 || corr.y != 0) {
			mScrollerLastX = mScrollerLastY = 0;
			mScroller.startScroll(0, 0, corr.x, corr.y, SCROLL_DURATION);
			mStepper.prod();
		}
	}
